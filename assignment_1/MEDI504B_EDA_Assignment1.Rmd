...
```{r, eval=FALSE}
# Automated EDA
create_report(DiabetesHIDataSet_train_unique)
```
...
> Based on the histograms generated, we also found some outliers in the `BMI` features. There are some variations in the 60+ range with a max BMI of 98.
...
> For both groups, the features were clustered into three groups. We can also see that the clusters were fairly similar to each other.
